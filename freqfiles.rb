...
def extract_keywords_mecab( str, method = :default )
   ...
   $KCODE = "e"
   lines = mecab.parse( str.downcase )
   $KCODE = "u"
   ...
   lines.each_with_index do |l, i|
      ...
      # するので、分割形も登録しておく。
      if l[0] =~ /・/o
         lines_separated = mecab.parse( NKF.nkf( "-Wem0", l[0].gsub( /・/o, " " ) ) )
         lines_separated = NKF.nkf( "-Ewm0", lines_separated ).split( /\n/ ).map{|l| l.split(/\t/) }.select{|w| w[2] and w[1] =~ /^名詞|UNK|形容詞/o and w[1] !~ /接[頭尾]|非自立|代名詞/o }
